...
def withdrawl_money(ac_no):
       for i in range(2,10):
            if customer_data[str(i)]['AC/NO']==ac_no:
                print(80*'-')
                print(f"{info_bullet} YOUR RECORD FOUND SUCCESSFULLY 👍\n")
                global amount_to_withdrawl
                amount_to_withdrawl = float(input(f"{green_bullet} ENTER AGAIN AMOUNT YOU WANT TO PAY : "))
                if amount_to_withdrawl<customer_data[str(i)]['PREVIOUS BALANCE']:
                    confirm_withdrawl = input(f"{green_bullet} PRESS 'CP' TO CONFIRM PAYMENT : ").upper()
                    if confirm_withdrawl == "CP":
                       print(f"\n{info_bullet} PAYMENT SUCCESSFULLY")
                    else:
                        print(f'{red_bullet} WITHDRAWL CANCELLED X')
                else:
                    print(f"{red_bullet} YOU CANNOT WITHDRAWL {amount_to_withdrawl} BECAUSE YOUR PREVIOUS BALANCE IS {customer_data[str(i)]['PREVIOUS BALANCE']}")
                    print(f"{info_bullet} PLEASE TRY AGAIN")
                print(80*'-')
            else:
                continue 
